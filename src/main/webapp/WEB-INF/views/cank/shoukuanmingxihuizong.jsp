			...
			<div class="layui-form-item">
				<div class="layui-input-block">
					...
					<button style="width: 10%;  float:right;"
						class="layui-btn layui-btn-normal" id="downLoad">数据导出</button>
				</div>
			</div>
		</form>
			...
			<table class="layui-table" lay-size="sm">
				<thead>
					<tr>
						...
						<th style="width:50px">经营地址</th>
						<th style="width:50px">商户号</th>
						<th style="width:50px">入驻时间</th>
						<th style="width:50px">渠道经理</th>
						<th style="width:50px">渠道经理联系方式</th>
						<th style="width:50px">累计收款次数</th>
						<th style="width:50px">累计收款金额</th>
						<th style="width:50px">昨天收款次数</th>
						<th style="width:50px">昨天收款金额</th>
						<th style="width:50px">渠道编码</th>
						<th style="width:50px">发展人编码</th>
						<th style="width:50px">能人身份证号码</th>
						<th style="width:50px">商户一级分类</th>
						<th style="width:50px">商户二级分类</th>
					</tr>
				</thead>
				<tbody>
		...
		layui.use([ 'jquery', 'laydate', 'form', 'layer', 'laypage', 'element' ], function() {
			...
			function addRow(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if (!curr.co) {
						curr.totalCount = '0';
						curr.totalMoney = '0.00';
						curr.dayCount = '0';
						curr.dayMoney = '0.00';
					}
					for (var key in curr.co) {
						if ("total_count" == key) {
							curr.totalCount = curr.co[key];
						} else if ("total_money" == key) {
							curr.totalMoney = curr.co[key];
						} else if ("day_count" == key) {
							curr.dayCount = curr.co[key];
						} else if ("day_money" == key) {
							curr.dayMoney = curr.co[key];
						}
					}
			...
			function getData(condition) {
				$.ajax({
					type : "post",
					url : "${pageContext.request.contextPath}/gatherDetailCollectData",
					async : false,
					beforeSend : function() {
			...
			$('#downLoad').click(function(){
				downLoadCondition.type = 0;
				$.ajax({
					type : "post",
					url : "${pageContext.request.contextPath}/addDownLoadTask",
					async : false,
